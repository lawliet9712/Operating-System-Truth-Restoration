...
uint32_t* pte_ptr(uint32_t vaddr) {
   ...
   uint32_t* pte = (uint32_t*)(0xffc00000 + \
	 ((vaddr & 0xffc00000) >> 10) + \
	 PTE_IDX(vaddr) * 4); /* PTE 只有 10 位，而这里把 PTE 放到页内偏移的位置（12位），所以需要 *4，即左移 2 位*/
   return pte;
}

/* 得到虚拟地址vaddr对应的pde的指针 */
/* 思路大致如下：页目录最后一项本身就是页表的地址，所以 PDE 获取到的还是页表，然后 PTE 获取的时候
 还是获取的最后一项，那么又获取到了页表的地址，接着只需要把 addr 解析一下他的 pde 位，算到页偏移项
 即可，这样虚拟地址按照 32bit 2 级页表的解析规则，最后还是得到 addr 的 pde*/
...
/* 用户内存池的位图紧跟在内核内存池位图之后 */
   ...
   /******************** 输出内存池信息 **********************/
   put_str("      kernel_pool_free page");put_int((int)kernel_free_pages);
   put_str("      user_free_pages:");put_int((int)user_free_pages);
      put_str("\n");
